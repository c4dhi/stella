...
get_var_metadata() {
    case "$1" in
        ...
        VAD_MIN_SPEECH_MS)     echo "stt|text|optional|300|200|Min speech duration (ms)||" ;;
        VAD_MIN_SILENCE_MS)    echo "stt|text|optional|800|400|Min silence before utterance end (ms)||" ;;
        PARTIAL_INTERVAL_MS)   echo "stt|text|optional|1000|500|Partial transcript interval (ms)||" ;;

        # --- TTS ---
        TTS_PROVIDER)          echo "tts|select|optional|edge_tts|kokoro|Text-to-speech engine|edge_tts,kokoro,chatterbox,elevenlabs,auto|" ;;
...
get_option_description() {
    ...
    case "${var_name}:${option}" in
        ...
        TTS_PROVIDER:kokoro)      echo "Fast local TTS (50-100ms, GPU-accelerated, EN only)" ;;
