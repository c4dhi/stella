"""
Adapter for converting Stella Light Agent state machine to SDK ProgressState.

This adapter translates the Stella-specific state machine model into the
generic SDK progress types, enabling the frontend task panel to display
Stella agent progress using the standard schema.

Mapping:
    Stella State Machine          ->  SDK Progress Types
    ─────────────────────────────────────────────────────
    Plan                         ->  ProgressState
...
from stella_light_agent.models.state_machine import Plan, State, StateType, Task, TaskStatus, DeliverableStatus


class ProgressAdapter:
    """
    Converts Stella Light Agent state machine state to generic SDK ProgressState.

    This allows the frontend to use the same task panel UI regardless of
    whether the agent uses Stella's state machine or a different approach.
    """

    @staticmethod
    def state_type_to_execution_mode(state_type: StateType) -> ExecutionMode:
        """Convert Stella StateType to SDK ExecutionMode."""
    ...
    def task_status_to_item_status(status: TaskStatus) -> ItemStatus:
        """Convert Stella TaskStatus to SDK ItemStatus."""
        mapping = {
    ...
    def deliverable_status_to_item_status(status: str) -> ItemStatus:
        """Convert Stella deliverable status string to SDK ItemStatus."""
        mapping = {
    ...
    def from_execution_state(
        ...
        """
        Convert a Stella ExecutionState to a generic ProgressState.

        Args:
            execution_state: The Stella agent's current execution state.
    ...
    def _state_to_group(cls, state: State, is_current: bool = False) -> ProgressGroup:
        """Convert a Stella State to a ProgressGroup."""
        items = []
