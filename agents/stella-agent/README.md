...
A full agent implementation using the STELLA Agent SDK. This agent implements the InputGate → ExpertPool → Aggregator pipeline for intelligent conversation handling.

## Overview

Stella Agent is a production-ready agent that:

- Uses the **stella-ai-agent-sdk** for communication with session-management
...
pip install -e ../stella-ai-agent-sdk

# Install stella-agent
pip install -e .
```
...
```bash
# Run the agent
python -m stella_agent --server localhost:50051 --config config/llm_config.json
```
...
### Expert Agents (`src/stella_agent/experts/`)
