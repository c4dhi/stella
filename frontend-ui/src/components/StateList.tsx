...
const ProcessingModeIcon = ({ type }: { type: StateType }) => {
  if (type === StateType.STRICT) {
    return (
      <div className="flex items-center gap-1" title="Sequential Processing">
        ...
        <span className="text-[8px] text-neutral-500 tracking-wider uppercase">Sequential</span>
      </div>
    )
  }
  ...
  return (
    <div className="flex items-center gap-1" title="Flexible Processing">
      <span className="text-[10px] text-neutral-600">🔄</span>
      <span className="text-[8px] text-neutral-500 tracking-wider uppercase">Flexible</span>
    </div>
  )
}
    ...
    case StateStatus.COMPLETED:
      return (
        <motion.div
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          className="w-3 h-3 rounded-full bg-neutral-900 flex items-center justify-center"
        >
...
const DeliverableStatusIcon = ({ status }: { status: DeliverableStatus }) => {
  switch (status) {
    ...
    case DeliverableStatus.PARTIAL:
      return <span className="text-yellow-600 text-[10px]">◐</span>
    case DeliverableStatus.SKIPPED:
      return <span className="text-neutral-400 text-[10px]">⊘</span>
    default:
      return <span className="text-neutral-400 text-[10px]">○</span>
  }
}
...
const DeliverableRow = ({ deliverable, index }: { deliverable: StateDeliverable; index: number }) => {
  ...
  return (
    <motion.div
      ...
      className="pl-6 border-l border-neutral-200/40"
    >
      <div
        className={`flex items-center justify-between p-2 rounded-md cursor-pointer transition-colors ${
          isSkipped ? 'bg-neutral-50/50 hover:bg-neutral-100/50 opacity-60' :
          hasValue ? 'bg-green-50/50 hover:bg-green-50' : 'hover:bg-neutral-50'
        }`}
        onClick={() => setShowDetails(!showDetails)}
      >
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <DeliverableStatusIcon status={deliverable.status} />
          <span className={`text-[10px] text-neutral-700 truncate ${isSkipped ? 'line-through text-neutral-400' : ''}`}>
            {deliverable.description}
            {deliverable.required && !isSkipped && <span className="text-red-500 ml-1">*</span>}
            {isSkipped && <span className="text-[8px] text-neutral-400 ml-1.5 bg-neutral-200 px-1 py-0.5 rounded uppercase font-medium">Skipped</span>}
          </span>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showDetails && hasValue && (
          ...
          >
            <div className="text-[10px] text-neutral-800 bg-neutral-50 p-2 rounded border">
              <strong>Value:</strong> {String(deliverable.value)}
            </div>

            {deliverable.reasoning && (
              <div className="text-[9px] text-green-700 bg-green-50 p-2 rounded border border-green-200">
                <strong>Why collected:</strong> {deliverable.reasoning}
              </div>
            )}

            {deliverable.acceptance_criteria && (
              <div className="text-[9px] text-neutral-600 bg-neutral-50 p-2 rounded border">
...
const TaskRow = ({ task, index, isInCurrentState }: { task: StateTask; index: number; isInCurrentState: boolean }) => {
  ...
  return (
    <motion.div
      ...
      className="pl-4 border-l border-neutral-200/60"
    >
      <div
        className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition-all border ${
          isSkipped
            ? 'bg-neutral-50/60 border-neutral-200/40 hover:bg-neutral-100/50 opacity-70'
            : task.status === TaskStatus.IN_PROGRESS
            ? 'bg-blue-50/50 border-blue-200/60 hover:bg-blue-50/70'
            : task.status === TaskStatus.COMPLETED
            ? 'bg-white border-neutral-200/50 hover:bg-neutral-50/50'
            : 'bg-white border-neutral-200/50 hover:bg-neutral-50/50'
        }`}
      ...
      >
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            <div className={`text-[11px] font-medium text-neutral-800 truncate ${isSkipped ? 'line-through text-neutral-400' : ''}`}>
              {task.description}
              {task.required && !isSkipped && <span className="text-red-500 ml-1">*</span>}
              {isSkipped && <span className="text-[9px] text-neutral-500 ml-2 bg-neutral-200 px-2 py-0.5 rounded-full uppercase font-medium">Skipped</span>}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {task.deliverables.length > 0 && (
            <span className="text-[8px] text-neutral-500 bg-neutral-100 px-1.5 py-0.5 rounded">
  ...
  return (
    <motion.div
      ...
      className={`rounded-lg border transition-all ${
        isCurrentState
          ? 'border-blue-400/70 bg-gradient-to-br from-blue-50/80 to-blue-100/40 shadow-md ring-1 ring-blue-200/50'
          : state.status === StateStatus.COMPLETED
          ? 'border-green-300/50 bg-green-50/30'
          : state.status === StateStatus.IN_PROGRESS
          ? 'border-yellow-300/50 bg-yellow-50/30'
          : 'border-neutral-200/60 bg-white/80 hover:bg-neutral-50/50'
      }`}
    >
      <div
        ...
        <AnimatePresence>
          {isExpanded && (
            <motion.div
              ...
              exit={{ opacity: 0, height: 0 }}
              transition={{ duration: 0.2 }}
              className="mt-3 pt-3 border-t border-neutral-200/50 space-y-2"
            >
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-[8px] text-neutral-500 tracking-wider uppercase">
      ...
      <AnimatePresence>
        {isExpanded && state.tasks.length > 0 && (
          <motion.div
            ...
            exit={{ opacity: 0, height: 0 }}
            transition={{ duration: 0.3 }}
            className="border-t border-neutral-200/50 p-4 pt-3 space-y-3"
          >
