...
export default function PlanBuilderSection() {
  ...
  const { openModal } = usePlanBuilderStore()
  ...
  const loadTemplates = async () => {
    try {
      setIsLoading(true)
      setError(null)
      const data = await apiClient.listPlanTemplates()
      setTemplates(data)
    } catch (err) {
  ...
  const handleCreate = () => {
    openModal(undefined, async () => {
      await loadTemplates()
    })
  }

  const handleEdit = (template: PlanTemplate) => {
    openModal(template, async () => {
      await loadTemplates()
    })
  }
  ...
  const handleDuplicate = async (template: PlanTemplate) => {
    try {
      const duplicated = await apiClient.duplicatePlanTemplate(template.id)
        ...
        >
          <div>
            ...
            <p className={`text-body-sm mt-1.5 max-w-lg ${
              ...
              Design structured plans for your AI agents with states, tasks, and deliverables
            </p>
          </div>
          ...
          {!isLoading && !error && templates.length === 0 && (
            ...
            >
              ...
              <p className={`text-body mb-8 max-w-sm mx-auto ${isDark ? 'text-content-inverse-secondary' : 'text-content-secondary'}`}>
                Create structured plans with states, tasks, and deliverables to guide your AI agents through complex workflows
              </p>
              <motion.button
                onClick={handleCreate}
